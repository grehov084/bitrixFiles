...
function first_column_position(elem_width){
		...
		for(elem of first_column_elems){
			...
			margin_top += elem.offsetHeight + marginBetween;
	...
	show_more.addEventListener("click", function(){
		if(window.innerWidth > 600){
			if(window.location.pathname == "/events/"){
				$.ajax({
					url: '/local/templates/main2/components/bitrix/news.list/news_main_list/ajax/more_events_list.php?PAGEN_1=' + newsPagen,
				...
				$.ajax({
					url: '/local/templates/main2/components/bitrix/news.list/news_main_list/ajax/more_news_list.php?PAGEN_1=' + newsPagen,
					success: function(data){
						$('.section-news-row').append(data);
						newsPagen++;
						counter = 0;
